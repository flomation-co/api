-- Seed the trigger_type row for the database-row poll trigger. Without it,
...
--
-- Pairs with launch migration 47, which adds 'database-row' to launch's
-- TriggerType enum. BOTH are required; the name must match the executor's trigger
-- id exactly. The api derives it by turning 'trigger/database_row' into
-- 'database-row' (underscores -> hyphens).
--
-- NUMBERING: 131. main is at 130 (130_AllowAssetBlobPurpose) when this is
...
SELECT 'database-row'
WHERE NOT EXISTS (
    SELECT 1 FROM trigger_type t WHERE t.name = 'database-row'
);
