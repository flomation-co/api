-- Remove the trello-webhook trigger_type seed row. Note the asymmetry with the
-- launch side: this seed row deletes cleanly, but launch's matching down
-- migration (34_AddTrelloTriggerType) is a no-op because Postgres can't drop an
...
DELETE FROM trigger_type WHERE name = 'trello-webhook';
