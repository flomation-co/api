-- Seed the trigger_type row for the salesforce-poll trigger. Without it,
...
--
-- Pairs with launch migration 51, which adds 'salesforce-poll' to launch's
-- TriggerType enum. BOTH are required; the name must match the executor's trigger
-- id exactly. The api derives it by turning 'trigger/salesforce_poll' into
-- 'salesforce-poll' (underscores -> hyphens).
--
-- NUMBERING: 139. main is at 138 (138_AddSalesforceProvider) when this is
-- written. golang-migrate SILENTLY SKIPS an out-of-order version and a DUPLICATE
-- version makes the api fail to BOOT (502). Git does not flag a clash (filenames
-- differ) and the Go tests do not run migrations against a real database, so
-- re-check this number AFTER rebasing on main, not just now.
--
...
SELECT 'salesforce-poll'
WHERE NOT EXISTS (
    SELECT 1 FROM trigger_type t WHERE t.name = 'salesforce-poll'
);
